...
func collectOn(t *testing.T, cw *CertWatch, addr string) []core.Finding {
	t.Helper()
...
func newCW(ca *testCA) *CertWatch {
	cw := New()
...
func TestCollect_SelfSignedIsUntrusted(t *testing.T) {
	// Self-signed, and CertWatch trusts only the (unrelated) test CA.
	ca := newTestCA(t)
...
// The full stack, end to end: scheduler drives CertWatch against a live local
...
func TestEndToEnd_SchedulerCertWatchStore(t *testing.T) {
	ca := newTestCA(t)
