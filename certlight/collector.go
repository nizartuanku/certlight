// Package certwatch is the first Hexward product: TLS/certificate expiry and
...
// moduleID stays "certwatch" (the original product id) so existing licenses,
// databases and integrations keep working across the CertLight rename.
const moduleID = "certwatch"

// CertLight implements core.Collector.
type CertLight struct {
...
// New returns a production-configured CertLight.
func New() *CertLight {
	return &CertLight{ProbeLegacyProtocols: true, now: time.Now}
}

func (c *CertLight) Describe() core.ModuleInfo {
...
func (c *CertLight) ValidateTarget(raw string) (core.Target, error) {
...
func (c *CertLight) Collect(ctx context.Context, t core.Target) ([]core.Finding, error) {
	host := t.Meta["host"]
...
func (c *CertLight) Diff(prev, cur []core.Finding) []core.Change { return nil }
...
func (c *CertLight) dial(ctx context.Context, addr, serverName string, minVer, maxVer uint16) (tls.ConnectionState, error) {
	d := tls.Dialer{
...
func (c *CertLight) checkExpiry(t core.Target, leaf *x509.Certificate, now time.Time) []core.Finding {
...
func (c *CertLight) checkHostname(t core.Target, leaf *x509.Certificate, host string) []core.Finding {
...
func (c *CertLight) checkChain(t core.Target, state tls.ConnectionState, now time.Time) []core.Finding {
	leaf := state.PeerCertificates[0]
...
func (c *CertLight) checkProtocol(t core.Target, state tls.ConnectionState) []core.Finding {
...
func (c *CertLight) probeLegacy(ctx context.Context, t core.Target, host string) []core.Finding {
...
func (c *CertLight) checkCipher(t core.Target, state tls.ConnectionState) []core.Finding {
...
func (c *CertLight) checkKeyStrength(t core.Target, leaf *x509.Certificate) []core.Finding {
...
func (c *CertLight) checkSignature(t core.Target, leaf *x509.Certificate) []core.Finding {
